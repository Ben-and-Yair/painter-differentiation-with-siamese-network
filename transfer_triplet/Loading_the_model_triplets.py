...
PATH = 'saved_models/model_500.pth'
...
def test(model, device, test_loader, name):
    ...
    with torch.no_grad():
        for batch_idx, (anchor, positive, negative) in enumerate(test_loader):
            ...
            pred = torch.where(subtract < 0, 1, 0)  # get the index of the max log-probability
...
def main():
    ...
    test_dataset = SiameseDatasetTriplets(X_test, y_test, length=X_test.shape[0] * 50)
